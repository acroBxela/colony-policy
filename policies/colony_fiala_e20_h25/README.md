# Colony Can-Am policy: colony_fiala_e20_h25

Self-contained CPU JAX/StableHLO policy and closed-loop model prediction.
Selected checkpoint: `step_000010092544`. Fine-tuned from `colony_fiala_e15_h10/pose:step_000455081984`.
This is the successful early checkpoint shown in drive_agent, not the later
collapsed checkpoints. Earlier working policies remain separate.
...
- Selected actor at 10,092,544 additional frames, initialized from the working
  455,081,984-frame recovery policy. Later updates degraded completion; they
...
- Starting s uniform over the full 96.2654824574 m lap, throughout training.
  Starting e uniform +/-2 m and heading relative to the tangent uniform +/-25 degrees.
  Initial sideslip and yaw rate zero; vx uniform [0.08,0.15] m/s throughout
  this fine-tune (the source already completed speed annealing).
...
- Reward: forward progress, minus dt times lateral error squared (weight 12),
  ...
  Finish bonus 20; failure penalty 100. Speed/ay limits are soft costs, not clamps.
- Independent 128-seed evaluation: 128/128 completed; mean lap 22.1099 s,
  RMS e 0.3804 m, peak |e| 2.1367 m, peak |ay| 5.7466 m/s².
...
Nominal result: 1063 samples, 21.2291 s lap, peak |e| 0.3040 m.
Surface reconstruction maximum x/y error: 0.0000063 m.
